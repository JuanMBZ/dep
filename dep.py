...
parser.add_argument("-c", "--column", help="Set column number of the whole part of EMP_PAY", default=70, type=int)
parser.add_argument("-z", "--zeroes", help="Add inputted number of zeroes to the left of EMP_ACCTNO", default=0, type=int)
parser.add_argument("-u", "--sheetname", action="store_true", help="Sets output file names to their correspinding sheet name. Default uses the excel file's name.")
...
for index, sheet in enumerate(args.sheet):
    try:
    ...
    if args.sheetname:
        out_name = excel_book.sheet_names[sheet] if isinstance(sheet, int) else sheet 
    else:
        if index == 0:
            out_name = path.basename(args.excel_file).split('.')[0]
        else:
            out_name = path.basename(args.excel_file).split('.')[0] + '_' + str(index)
    with open(out_name + ".dep", "w", encoding="utf-8") as f:
